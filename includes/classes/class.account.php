	...
	class ISC_ACCOUNT
	{
		...
		public function HandlePage()
		{
			...
			if (CustomerIsSignedIn()) {
				...
				$this_page = urlencode(sprintf("areadecliente.php?action=%s", $action));
				ob_end_clean();
		...
		*/
		private function ShowReturns()
		{
			...
			if (GetConfig('EnableReturns') == 0) {
				// Bad details
				ob_end_clean();
				header(sprintf("location:%s/areadecliente.php", $GLOBALS['ShopPath']));
				die();
			}
		...
		private function SaveNewShippingAddress()
		{
			...
			if (isset($ShippingAddress['shipfirstname']) && isset($ShippingAddress['shipaddress1'])) {
				...
				if (isId($shippingid)) {
					...
					// Database error
					ob_end_clean();
					header(sprintf("location:%s/%s", $GLOBALS['ShopPath'], 'areadecliente.php?action=add_shipping_address'));
					die();
				}
			}
			else {
						...
						if (count($data_vals) == 6) {
							...
							while ($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
								...
								if ($expired == true) {
									...
									$GLOBALS['DownloadName'] = sprintf("<a href=\"%s/areadecliente.php?action=download_item&data=%s\">%s</a>", $GLOBALS['ShopPathSSL'], $GLOBALS['DownloadItemEncrypted'], $GLOBALS['DownloadName']);
...
// fim
		...
		*/
		...
		private function AddressBook()
		{
			...
			$GLOBALS['FromURL'] = urlencode("areadecliente.php?action=address_book");
			$GLOBALS['HideAddressButton'] = "none";
			$GLOBALS['CheckoutShippingTitle'] = GetLang('YourAddressBook');
			$GLOBALS['CheckoutShippingIntro'] = sprintf("%s <a href='%s/areadecliente.php?action=add_shipping_address&amp;address_type=&amp;from=%s'>%s</a>", GetLang('AddressBookIntro1'), $GLOBALS['ShopPath'], $GLOBALS['FromURL'], GetLang('AddressBookIntro2'));

			$GLOBALS['CheckoutShippingIntroNoAddresses'] = sprintf("%s <a href='%s/areadecliente.php?action=add_shipping_address&amp;address_type=&amp;from=%s'>%s</a>", GetLang('AddressBookIntro1NoAddresses'), $GLOBALS['ShopPath'], $GLOBALS['FromURL'], GetLang('AddressBookIntro2NoAddresses'));
		...
		*/
		private function DoReorder()
		{
			...
			if(!empty($UnreorderableProducts)) {
				...
				ob_end_clean();
				header(sprintf("Location: %s/areadecliente.php?action=view_order&order_id=%s&reorder=1", $GLOBALS['ShopPath'], $OrderId));
			} else {
				ob_end_clean();
