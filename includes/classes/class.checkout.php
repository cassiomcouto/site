	...
	*/
	...
	private function SaveMultipleShippingAddresses()
	{
		...
		if(isset($_POST['addAnotherAddress'])) {
			...
			@ob_end_clean();
			header(sprintf("Location: %s/areadecliente.php?action=add_shipping_address&from=%s", $GLOBALS['ShopPath'], $this_page));
			die();
		}
	...
	private function ChooseShippingAddress($errors = array())
	{
		...
		if(!CustomerIsSignedIn()) {
			...
			$GLOBALS['CheckoutShippingIntro'] = sprintf("%s <a href='%s/areadecliente.php?action=add_shipping_address&amp;address_type=" . FORMFIELDS_FORM_SHIPPING . "&amp;from=%s'>%s</a>", GetLang('ChooseShippingAddressIntro1'), $GLOBALS['ShopPath'], $GLOBALS['FromURL'], GetLang('ChooseShippingAddressIntro2'));
			$GLOBALS['CheckoutMultiShippingIntro'] = sprintf("%s <a href='%s/areadecliente.php?action=add_shipping_address&amp;address_type=" . FORMFIELDS_FORM_SHIPPING . "&amp;from=%s' onclick='Checkout.MultiAddNewAddress(\"shipping\"); return false;'>%s</a>", GetLang('ChooseMultiShippingAddressIntro1'), $GLOBALS['ShopPath'], $GLOBALS['FromURL'], GetLang('ChooseMultiShippingAddressIntro2'));
		}
...
// 			$GLOBALS['HideFullNameField'] = 'display: none';
			...
			$GLOBALS['CheckoutShippingIntro'] = sprintf("%s <a href='%s/areadecliente.php?action=add_shipping_address&amp;from=%s'>%s</a>", GetLang('ChooseBillingAddressIntro1'), $GLOBALS['ShopPath'], $GLOBALS['FromURL'], GetLang('ChooseBillingAddressIntro2'));
